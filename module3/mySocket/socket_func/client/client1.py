...
download_path = r"/Users/yangyiyi/Documents/oldboy/module3/mySocket/socket_func/file_client/download"


def get_file(server_socket, file_name):
...
def put_file(server_socke, file_name):
...
def run():
    ...
    server_socket.connect(ip_port)  # 主动初始化TCP服务器连接
    while True:
        methods = {"get": get_file, "put": put_file}
        msg = input(">>>>:").strip()
        if re.fullmatch(r"(get|put){1}\s{1}\S+", msg, re.I):
            ...
            server_socket.send(header_size)
            server_socket.send(header_bytes)
            server_socket.send(msg_bytes)  # 发送的数据必须是bytes类型
            print(file_name)
            methods[file_method](server_socket, file_name)
        else:
            print("输入格式不正确，请重新输入！")
    server_socket.close()
