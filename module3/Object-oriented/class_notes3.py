...
# 当函数属性用装饰器abc.abstractmethod獐狮的时候，子类必须按照抽象类的定义实现这个固定的函数属性，否则报错。
'''
import abc #需要用到abc这个模块来定影抽象类
...
#多态性：指的是可以在不考虑对象的类型的情况下而直接使用对象  从而扩展性高
peo1=People()
dog1=Dog()
pig1=Pig()
cat1=Cat()
...
# 鸭子类型  不同类里面定义的函数或者数据属性名字相同，不同对象在调用这些属性的时候，看上去都是一样的属性，实现功能肯跟也都是一样，但内部代码逻辑并不一样。
class Disk:
    def read(self):
        print('disk read')
